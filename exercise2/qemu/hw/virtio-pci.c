 ...
 *  Dimitris Siakavaras <jimsiak@codesourcery.com>
 *
...
/***************************** virtio-crypto ************************************/
/*
 * The following section is used for virtio-crypto-serial-pci device. 
 * The differences against virtio-serial-pci are the following:       
 *   1. We now call virtio_crypto_serial_init and  
 *      virtio_crypto_serial_exit functions for vdev initialization.
 */
static int virtio_crypto_serial_init_pci(PCIDevice *pci_dev)
{
    VirtIOPCIProxy *proxy = DO_UPCAST(VirtIOPCIProxy, pci_dev, pci_dev);
    VirtIODevice *vdev;
    ...
    vdev = virtio_crypto_init(&pci_dev->qdev);
    if (!vdev) {
        return -1;
    }
    vdev->nvectors = proxy->nvectors == DEV_NVECTORS_UNSPECIFIED
...
static void virtio_crypto_serial_exit_pci(PCIDevice *pci_dev)
{
    VirtIOPCIProxy *proxy = DO_UPCAST(VirtIOPCIProxy, pci_dev, pci_dev);

    virtio_pci_stop_ioeventfd(proxy);
    virtio_crypto_exit(proxy->vdev);
    virtio_exit_pci(pci_dev);
}

static Property virtio_crypto_serial_properties[] = {
    DEFINE_PROP_BIT("ioeventfd", VirtIOPCIProxy, flags, VIRTIO_PCI_FLAG_USE_IOEVENTFD_BIT, true),
    DEFINE_PROP_UINT32("vectors", VirtIOPCIProxy, nvectors, DEV_NVECTORS_UNSPECIFIED),
    DEFINE_PROP_HEX32("class", VirtIOPCIProxy, class_code, 0),
    DEFINE_VIRTIO_COMMON_FEATURES(VirtIOPCIProxy, host_features),
...
static void virtio_crypto_serial_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    PCIDeviceClass *k = PCI_DEVICE_CLASS(klass);

    k->init = virtio_crypto_serial_init_pci;
    k->exit = virtio_crypto_serial_exit_pci;
    k->vendor_id = PCI_VENDOR_ID_REDHAT_QUMRANET;
    k->device_id = PCI_DEVICE_ID_VIRTIO_CRYPTO;
    k->revision = VIRTIO_PCI_ABI_VERSION;
    k->class_id = PCI_CLASS_COMMUNICATION_OTHER;
    dc->reset = virtio_pci_reset;
    dc->props = virtio_crypto_serial_properties;
}

static TypeInfo virtio_crypto_serial_info = {
    .name          = "virtio-crypto-pci",
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(VirtIOPCIProxy),
    .class_init    = virtio_crypto_serial_class_init,
};
/**********************************************************************/
...
static void virtio_pci_register_types(void)
{
    ...
    /* virtio-crypto */
    /* Also register "virtio-crypto-serial-pci" device. */
    type_register_static(&virtio_crypto_serial_info);
}
